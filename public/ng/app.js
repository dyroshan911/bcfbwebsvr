...
myApp.config(['$routeProvider', '$locationProvider', '$resourceProvider',
	function ($routeProvider, $locationProvider, $resourceProvider) {
		...
		$routeProvider.when('/wechat-insurance', { templateUrl: ('partial/wechat-insurance'), controller: 'WechatInsuranceCtrl', permission: 'wechat' });
		$routeProvider.when('/wechat-works', { templateUrl: ('partial/wechat-works'), controller: 'WechatWorksCtrl', permission: 'wechat' });
		...
		$routeProvider.when('/wechat-policy', { templateUrl: ('partial/wechat-policy'), controller: 'WechatPolicyCtrl', permission: 'wechat' });
			...
			SessionService.initSession(function (data) {
				...
				if ((args.code && args.state) || args.openid) {
					...
					SessionService.wechatAuth(args, function (res) {
						...
						firstStart = false;
						if (path.indexOf('wechat-signup') != -1 || path.indexOf('wechat-insurance') != -1 || path.indexOf('wechat-works') != -1 || path.indexOf('wechat-apply') != -1) {
							$route.reload();
						} else {
